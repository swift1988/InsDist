...
class fgdloss(nn.Module):
    def __init__(self,
        ...
        self.conv_mask_s = nn.Conv2d(teacher_channels, 1, kernel_size=1).cuda()
        self.conv_mask_t = nn.Conv2d(teacher_channels, 1, kernel_size=1).cuda()
        
        self.channel_add_conv_s = nn.Sequential(
            nn.Conv2d(teacher_channels, teacher_channels//2, kernel_size=1),
            nn.LayerNorm([teacher_channels//2, 1, 1]),
            nn.ReLU(inplace=True),  # yapf: disable
            nn.Conv2d(teacher_channels//2, teacher_channels, kernel_size=1)).cuda()
